...
if env_type == "HESTIA" :
	...
	if generate_base == 'true':
		file_lg_name = 'lg_candidates_LGFnew_'+resolution+'.txt'
		file_lare_name = 'lg_candidates_LaReLGFnew_'+resolution+'.txt'
		file_codes_name = 'lg_codes_'+resolution+'.txt'
	else:
		file_lg_name = 'lg_candidates_LGFold_'+resolution+'.txt'
		file_lare_name = 'lg_candidates_LaReLGFold_'+resolution+'.txt'
		file_codes_name = 'lg_codes_'+resolution+'.txt'
...
for irun in range(lss_init, lss_end):
	...
	if env_type == "HESTIA" :
		...
		# LaRe finder - script
		lare_find_sh = '/z/carlesi/CLUES/GridProperties/scripts/lare_find.sh'

		# LaRe generator - ginnungagap script
		lare_gen_sh = '/z/carlesi/CLUES/ginnungagap/ginnungagap/zoom/gen_mask.sh'
		lare_dir_sh = '/z/carlesi/CLUES/ginnungagap/ginnungagap/zoom/'
	else:
		...
		# LaRe finder - script
		lare_find_sh = '/home/eduardo/CLUES/GridProperties/scripts/lare_find.sh'

		# LaRe generator - ginnungagap script
		lare_gen_sh = '/home/eduardo/CLUES/ginnungagap/zoom/gen_mask.sh'
		lare_dir_sh = '/home/eduardo/CLUES/ginnungagap/zoom/'
	...
	for run_i in range(run_init, run_end):
		...
		if os.path.exists(file_ahf_in):
			...
			#	print 'Total mass within %.3f kpc/h = %.2e' % (virgo_r, mtot)
			#	print 'New virgo center and mass ', m0, x0
			...
			(base_run, lg1.ID, lg2.ID, lg1.m/h0, lg2.m/h0, d12/h0, vel_r, lg_com[0]/facMpc, lg_com[1]/facMpc, lg_com[2]/facMpc)
					...
					if fac_lg12 < fac_lg0 and m12 < m_max:

						# Only append the best pair
						all_lgs.append(lg1)
						all_lgs.append(lg2)
						...
						if generate_lare == "true" and vel_r < vrad_max and m12 < m_max:
							# Only add the LaRe if for the likeliest pair - in case it will be overwritten
							lare_r = extra_r + d12 + lg1.r + lg2.r
							#print 'Vel Rad: %f ' % (vel_r) 
							#print 'LareR: %f, R1: %f, R2: %f, d12: %f, extra: %f' % \
							#(lare_r, lg1.r, lg2.r, d12, extra_r)
							...
							(base_run, lare_r/facMpc, lg_com[0]/facMpc, lg_com[1]/facMpc, lg_com[2]/facMpc)
							...
							if os.path.exists(file_ic_in) or os.path.exists(file_ic_in + '.0'):
								...
								all_lare_x = find_lare(lg_com, lare_r, file_ic_in, file_z0_in, \
